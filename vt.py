...
def match_ssn(ssn, df1_ssn, df2_ssn):
    ...
    if possible_matches.empty:
        ...
        print(f"Error processing SSN {ssn}: {e}")
        return pd.DataFrame(columns=['SSN_N', 'Payment Date', 'SUM(TXN_CASH_A)'])
...
result_list = []
with ThreadPoolExecutor() as executor:  # Use ProcessPoolExecutor for better parallelization
    futures = []
