...
# Ensure 'available' column exists in df2
if 'available' not in df2.columns:
    df2['available'] = True
...
unmatched_exact.drop(columns=['_merge', 'TXN_TRD_D'], errors='ignore', inplace=True)

# **Step 2: Find Closest Date Matches Within 28 Days for Unmatched Rows**

if not unmatched_exact.empty:
    # Prepare unmatched df1 rows
    unmatched_exact = unmatched_exact.reset_index().rename(columns={'index': 'index_df1'})
    ...
    df2_reset = df2.reset_index().rename(columns={'index': 'index_df2'})
    df2_reset['key'] = df2_reset['SSN_N']
    
    # Only consider available df2 rows
    df2_available = df2_reset[df2_reset['available']].copy()
    ...
    if not potential_matches.empty:
        ...
        potential_matches_unique_df2 = potential_matches.drop_duplicates(subset=['index_df2'])
        
        # From the above, ensure that each df1 row gets the best available df2 row
        potential_matches_unique = potential_matches_unique_df2.drop_duplicates(subset=['index_df1'])
        
        # Get indices of matched df1 and df2 rows
        matched_indices_df1 = potential_matches_unique['index_df1'].tolist()
        matched_indices_df2 = potential_matches_unique['index_df2'].tolist()
        
        # Update availability in df2
        df2.loc[matched_indices_df2, 'available'] = False
        
        # Prepare matched and unmatched dataframes
        matched_closest = potential_matches_unique.copy()
        unmatched_final = unmatched_exact[~unmatched_exact['index_df1'].isin(matched_indices_df1)].copy()
    else:
...
final_result.drop(columns=[
    'Payment_Date_minus_28', 'Payment_Date_plus_28', 'date_diff',
    'key', 'index_df1', 'index_df2', 'available'
